...
def sync_network_carriers(network: Network) -> None:
    ...
    if added_count > 0:
        print(
            f"   Automatically added {added_count} missing carriers from network components"
        )


def get_class_enum_from_string(class_name: str) -> ClassEnum | None:
...
def discover_multi_sector_classes(db: PlexosDB) -> dict[str, list[str]]:
    ...
    """
    multi_sector_classes = {"gas": [], "flow": [], "process": [], "facility": []}
...
def get_object_properties_by_name(
    ...
    try:
        ...
        if class_enum is None:
            logger.debug(
                f"No ClassEnum mapping found for class '{class_name}', skipping properties for {object_name}"
            )
            return properties
        ...
        logger.debug(
            f"Found {len(properties)} properties for object {object_name} (class: {class_name})"
        )

    except Exception as e:
        logger.debug(
            f"Failed to get properties for object {object_name} (class: {class_name}): {e}"
        )
...
def get_object_memberships(
    ...
    try:
        # Use the built-in PlexosDB method
        memberships_system = db.get_memberships_system(
...
def load_csv_costs(inputs_folder: str) -> dict[str, pd.DataFrame]:
    ...
    try:
        ...
        build_costs_path = str(Path(inputs_folder) / "BuildCosts.csv")
        if Path(build_costs_path).exists():
            cost_data["build_costs"] = pd.read_csv(build_costs_path, index_col="YEAR")
            logger.info(
                f"Loaded BuildCosts.csv with {len(cost_data['build_costs'].columns)} technology-region combinations"
            )

        # Load fixed O&M costs
        foms_path = str(Path(inputs_folder) / "FOMs.csv")
        if Path(foms_path).exists():
            cost_data["foms"] = pd.read_csv(foms_path, index_col="YEAR")
            logger.info(
                f"Loaded FOMs.csv with {len(cost_data['foms'].columns)} technology-region combinations"
            )

        # Load variable O&M costs
        voms_path = str(Path(inputs_folder) / "VOMs.csv")
        if Path(voms_path).exists():
            cost_data["voms"] = pd.read_csv(voms_path, index_col="YEAR")
            logger.info(
                f"Loaded VOMs.csv with {len(cost_data['voms'].columns)} technology-region combinations"
            )

        # Load fuel prices with emissions
        fuel_prices_path = str(Path(inputs_folder) / "prices_w_emissions_permwh.csv")
        if Path(fuel_prices_path).exists():
            cost_data["fuel_prices"] = pd.read_csv(fuel_prices_path)
            logger.info(
                f"Loaded fuel prices with {len(cost_data['fuel_prices'].columns)} fuel types"
            )
...
def load_time_series_data(inputs_folder: str) -> dict[str, pd.DataFrame]:
    ...
    try:
        ...
        load_path = str(Path(inputs_folder) / "Load.csv")
        if Path(load_path).exists():
            load_df = pd.read_csv(
                load_path, parse_dates=["Datetime"], index_col="Datetime"
            )
            time_series_data["electricity_demand"] = load_df
            logger.info(
                f"Loaded Load.csv with {len(load_df.columns)} regions and {len(load_df)} time steps"
            )

        # Load hydrogen demand
        h2_demand_path = str(Path(inputs_folder) / "H2_Demand_With_Blending.csv")
        if Path(h2_demand_path).exists():
            h2_df = pd.read_csv(
                h2_demand_path, parse_dates=["Datetime"], index_col="Datetime"
            )
            time_series_data["hydrogen_demand"] = h2_df
            logger.info(f"Loaded H2 demand with {len(h2_df.columns)} regions")

        # Load VRE profiles
        solar_path = str(Path(inputs_folder) / "Solar_Inverted.csv")
        if Path(solar_path).exists():
            solar_df = pd.read_csv(
                solar_path, parse_dates=["Datetime"], index_col="Datetime"
            )
            time_series_data["solar_profiles"] = solar_df
            logger.info(f"Loaded Solar profiles with {len(solar_df.columns)} regions")

        wind_path = str(Path(inputs_folder) / "Wind_Inverted.csv")
        if Path(wind_path).exists():
            wind_df = pd.read_csv(
                wind_path, parse_dates=["Datetime"], index_col="Datetime"
            )
        ...
        hydro_path = str(Path(inputs_folder) / "Hydro.csv")
        if Path(hydro_path).exists():
...
def load_infrastructure_data(inputs_folder: str) -> dict[str, pd.DataFrame]:
    ...
    try:
        ...
        pipelines_path = str(Path(inputs_folder) / "H2Pipelines.csv")
...
def get_technology_costs(
    cost_data: dict[str, pd.DataFrame], technology: str, region: str, year: int = 2050
) -> dict[str, float]:
    ...
    try:
        ...
        if (
            "build_costs" in cost_data
            and tech_region_col in cost_data["build_costs"].columns
            and year in cost_data["build_costs"].index
        ):
        ...
        if (
            "foms" in cost_data
            and tech_region_col in cost_data["foms"].columns
            and year in cost_data["foms"].index
        ):
        ...
        if (
            "voms" in cost_data
            and tech_region_col in cost_data["voms"].columns
            and year in cost_data["voms"].index
        ):
...
def create_multi_sector_buses(
    network: Network, regions: list[str]
) -> dict[str, list[str]]:
    ...
    """
    sector_buses = {"electricity": [], "hydrogen": [], "ammonia": [], "gas": []}
    ...
    logger.info(
        f"Created multi-sector buses: {len(sector_buses['electricity'])} electricity, "
...
def create_sector_coupling_links(
    network: Network,
    ...
    for i, elec_bus in enumerate(electricity_buses):
        ...
        if i < len(hydrogen_buses):
            ...
            electrolysis_costs = get_technology_costs(
                cost_data, "Electrolysis_fac", region
            )
        ...
        if i < len(ammonia_buses):
            ...
            if h2_bus:
                ...
                crack_costs = get_technology_costs(
                    cost_data, "AmmoniaCrack_fac", region
                )
...
def add_multi_sector_storage(
    network: Network, sector_buses: dict[str, list[str]]
) -> dict[str, int]:
...
def setup_gas_electric_network_db(
    network: Network,
    db: PlexosDB,
    ...
    try:
        ...
        print(
            f"   Added {len(node_objects)} electricity buses and {len(snapshots)} snapshots"
        )
        ...
        if generators_as_links:
            ...
            generator_links = [
                ...
                if network.links.at[link, "bus0"] in fuel_buses
                and network.links.at[link, "bus1"] in elec_buses
            ]
            setup_summary["sector_coupling"]["generator_links"] = len(generator_links)
            print(
                f"   Created {len(generator_links)} generator-links and {len(network.generators)} generators"
            )
        else:
        ...
        gas_summary = port_gas_components(
            network=network,
            db=db,
            timeslice_csv=timeslice_csv,
        ...
        coupling_stats = add_gas_electric_coupling_db(
            network, db, generators_as_links=generators_as_links
        )
        ...
        if coupling_stats.get("gas_plants_added", 0) > 0:
            print(
                f"   Gas plants (from gas.py): {coupling_stats['gas_plants_added']} links"
            )
        ...
        if coupling_stats.get("sector_coupling_links", 0) > 0:
            print(
                f"   Multi-sector links (electrolysis/fuel_cell): {coupling_stats['sector_coupling_links']} links"
            )
        ...
        total_gen_capacity = (
            sum(network.generators.p_nom)
            if len(network.generators) > 0
            else len(elec_buses) * 1000
        )

        if generators_as_links:
            # Include generator-link capacity in total calculation
            fuel_buses = [bus for bus in network.buses.index if "fuel_" in bus]
            gen_links = network.links[
                (network.links.bus0.isin(fuel_buses))
                & (network.links.bus1.isin(elec_buses))
            ]
            gen_link_capacity = (
                sum(gen_links.p_nom * gen_links.efficiency) if len(gen_links) > 0 else 0
            )
            total_gen_capacity += gen_link_capacity

        load_per_bus = (
            total_gen_capacity / len(elec_buses) * 0.3 if len(elec_buses) > 0 else 100
        )

        for _i, bus in enumerate(elec_buses):
            load_name = f"elec_load_{bus}"
            if load_name not in network.loads.index:
                rng = np.random.default_rng()
                base_load = load_per_bus * (0.8 + 0.4 * rng.random())
...
def add_gas_electric_coupling_db(
    network: Network, db: PlexosDB, generators_as_links: bool = False
) -> dict[str, Any]:
    ...
    try:
            ...
            for gen_name in generator_objects:
                ...
                for membership in memberships:
                    if membership.get("class") == "Node":
                        ...
                        "gas" in membership.get("class", "").lower()
                        and "node" in membership.get("class", "").lower()
                    ):
                        ...
                        try:
                            ...
                            if link_name not in network.links.index:
                                network.add(
                                    ...
                                    marginal_cost=0.01,
                                )  # Small marginal cost for gas conversion
        ...
        hydrogen_buses = [
            bus
            for bus in network.buses.index
            if network.buses.at[bus, "carrier"] == "hydrogen"
        ]
        elec_buses = [
            bus
            for bus in network.buses.index
        ...
        if hydrogen_buses and elec_buses:
            ...
            for i, (elec_bus, h2_bus) in enumerate(
                zip(elec_buses[:3], hydrogen_buses[:3], strict=False)
            ):
            ...
            for i, (h2_bus, elec_bus) in enumerate(
                zip(hydrogen_buses[:2], elec_buses[:2], strict=False)
            ):
        ...
        total_coupling = (
            coupling_stats["gas_generators"]
            + coupling_stats["gas_plants_added"]
            + coupling_stats["sector_coupling_links"]
        )

        logger.info(
            f"Enhanced gas-electric coupling complete: {total_coupling} total coupling links"
        )
...
def setup_enhanced_flow_network_with_csv(
    network: Network,
    db: PlexosDB,
    inputs_folder: str,
    testing_mode: bool = False,
    ...
    try:
        ...
        if cost_data or time_series_data:
            ...
            print(
                f"   Loaded {len(cost_data)} cost files, {len(time_series_data)} time series files"
            )
        ...
        if not regions:
            ...
            regions = [
                bus.replace("_electricity", "").replace("_Electric", "")
                for bus in existing_buses
            ]
            regions = list(set(regions))  # Remove duplicates

        print(
            f"   Found {len(regions)} regions: {regions[:5]}..."
            if len(regions) > 5
            else f"   Found {len(regions)} regions: {regions}"
        )
        ...
        loads_added = add_multi_sector_loads_from_csv(
            network, sector_buses, time_series_data
        )
        ...
        if "h2_pipelines" in infrastructure_data:
            ...
            h2_links = add_h2_pipeline_network(
                network, infrastructure_data["h2_pipelines"], sector_buses
            )
        ...
        logger.exception("Error setting up enhanced flow network")
        raise
...
def add_multi_sector_loads_from_csv(
    network: Network,
    sector_buses: dict[str, list[str]],
        ...
        for nh3_bus in sector_buses.get("ammonia", []):
            ...
            if load_name not in network.loads.index and len(network.snapshots) > 0:
                ...
                demand_profile = pd.Series(
                    [base_demand] * len(network.snapshots), index=network.snapshots
                )
                network.add(
                    "Load",
                    load_name,
...
def add_h2_pipeline_network(
    network: Network, pipelines_df: pd.DataFrame, sector_buses: dict[str, list[str]]
) -> int:
                ...
                if "-" in regions_part:
                    ...
                    if len(parts) >= 4:  # AF-AGO-AF-COD format
                        ...
                        if (
                            source_bus
                            and dest_bus
                            and source_bus in network.buses.index
                            and dest_bus in network.buses.index
                        ):
                            ...
                            if link_name not in network.links.index:
                                ...
                                capital_cost = (
                                    float(pipeline.get("Build Cost", 1000000)) / 1000
                                )  # Convert to per MW
                                fixed_cost = (
                                    float(pipeline.get("FO&M Charge", 10000)) / 1000
                                )  # Convert to per MW/year
                                efficiency = (
                                    float(pipeline.get("Efficiency", 98)) / 100
                                )  # Convert to decimal
...
def add_processes_db(
    network: Network, db: PlexosDB, multi_sector_classes: dict[str, list[str]]
) -> dict[str, int]:
...
def add_facilities_db(
    network: Network,
    db: PlexosDB,
    multi_sector_classes: dict[str, list[str]],
    testing_mode: bool = False,
) -> dict[str, int]:
        ...
        if total_facilities > 0:
            ...
            for facility_class in multi_sector_classes.get("facility", []):
                ...
                for facility_name in facility_objects:
                    ...
                    memberships = get_object_memberships(
                        db, facility_class, facility_name
                    )
                    ...
                    processes = [
                        m["name"]
                        for m in memberships
                        if m.get("class", "").lower() == "process"
                    ]

                    # Debug logging - log more variety
                    if (
                        facility_index < 5 or facility_index % 200 == 0
                    ):  # Log first 5 + every 200th
                        print(f"    Debug - Facility {facility_index}: {facility_name}")
                        print(f"    Flow nodes found: {flow_nodes}")
                        print(f"    Processes found: {processes}")
                        if facility_index < 5:
                            print(
                                f"    Available buses: {list(network.buses.index)[:5]}..."
                            )  # First 5 buses

                    if len(flow_nodes) >= 1:
                        primary_bus = flow_nodes[
                            0
                        ]  # Use first flow node as primary bus
                        process_name = (
                        ...
                        is_generator = any(
                            gen_type in facility_name.lower()
                            or gen_type in process_name.lower()
                        ...
                        if facility_index < 5 or facility_index % 200 == 0:
                            print(
                                f"    Primary bus: {primary_bus}, Bus exists: {primary_bus in network.buses.index}"
                            )
                                ...
                                if link_name not in network.links.index:
                                    ...
                                    if (
                                        ...
                                        facility_stats["generators"] += (
                                            1  # Count as generator equivalent
                                        )
...
def setup_marei_csv_network(
    ...
    try:
        ...
        setup_summary["electricity"]["loads"] += demand_stats.get(
            "electricity_loads", 0
        )
        ...
        gas_infra_stats = add_marei_gas_infrastructure(
            network, marei_data, eu_countries
        )
        ...
        logger.exception("Error setting up MaREI CSV network")
        raise

    return setup_summary


def add_marei_csv_loads(
    network: Network, marei_data: dict, eu_countries: list[str]
) -> dict[str, int]:
                    ...
                    if load_name not in network.loads.index:
                        ...
                        if len(network.snapshots) == len(demand_profile):
                            demand_series = pd.Series(
                                demand_profile.values, index=network.snapshots
                            )
                        else:
                            # Resample or extend to match snapshots
                            demand_series = pd.Series(
                                [
                                    demand_profile.iloc[i % len(demand_profile)]
...
def add_marei_gas_infrastructure(
    network: Network, marei_data: dict, eu_countries: list[str]
) -> dict[str, int]:
        ...
        if "infrastructure" in marei_data and "flow" in marei_data["infrastructure"]:
            ...
            if not flow_data.empty and len(flow_data.columns) > 1:
                ...
                pipeline_connections = [
                    col for col in flow_data.columns if "-" in col and col != "Year"
                ]
                ...
                for connection in pipeline_connections:
                    if "-" in connection:
                        ...
                        if len(parts) == 2:
                            ...
                            if (
                                gas_bus1 in network.buses.index
                                and gas_bus2 in network.buses.index
                            ):
                                ...
                                if pipeline_name not in network.links.index:
                                    ...
                                    if len(flow_data) > 0:
                                        capacity_value = (
                                            flow_data[connection].iloc[0]
                                            if not flow_data[connection].empty
                                            else 100
                                        )
                                        try:
                                            capacity = (
                ...
                if storage_type in marei_data["infrastructure"]:
                    ...
                    if not storage_data.empty:
                        ...
                        storage_locations = [
                            ...
                            if col not in ["Year"] and len(col) <= 3
                        ]
                        ...
                        for country in storage_locations:
                            ...
                            if gas_bus in network.buses.index:
                                ...
                                if storage_name not in network.stores.index:
                                    ...
                                    if (
                                        len(storage_data) > 0
                                        and country in storage_data.columns
                                    ):
                                        cap_value = (
                                            storage_data[country].iloc[0]
                                            if not storage_data[country].empty
                                            else 1000
                                        )
                                        try:
                                            capacity = (
                                                float(cap_value) if cap_value else 1000
                                            )
                                        except Exception:
                                            capacity = 1000
                    ...
                    if gas_bus in network.buses.index:
                        ...
                        if lng_name not in network.stores.index:
                            ...
                            if country in lng_data.columns and len(lng_data) > 0:
                                cap_value = (
                                    lng_data[country].iloc[0]
                                    if not lng_data[country].empty
                                    else 500
                                )
                                try:
...
def add_marei_sector_coupling(
    network: Network, db: PlexosDB, generators_as_links: bool
) -> dict[str, Any]:
    ...
    try:
        ...
        gas_buses = [
            bus
            for bus in network.buses.index
            if network.buses.at[bus, "carrier"] == "gas"
        ]
        elec_buses = [
            bus
            for bus in network.buses.index
        ...
        if generators_as_links:
            ...
            for gen_name in generator_objects[:10]:  # Process first 10 for performance
                ...
                for prop in props:
                    if (
                        prop.get("property") == "Fuel"
                        and "gas" in str(prop.get("value", "")).lower()
                    ):
                        gas_connected = True
                        break
...
def apply_marei_gas_pricing(
    network: Network, pricing_data: dict, pricing_scheme: str
) -> None:
    ...
    try:
        ...
        if len(pricing_df) > 0 and "price" in pricing_df.columns:
            base_gas_price = (
                float(pricing_df["price"].iloc[0])
                if not pricing_df["price"].empty
                else 30.0
            )
        ...
        for link in gas_links:
            if network.links.at[link, "carrier"] in [
                "gas_transport",
                "gas_to_electricity",
            ]:
                current_cost = network.links.at[link, "marginal_cost"]
                network.links.at[link, "marginal_cost"] = max(
                    current_cost, base_gas_price * 0.01
                )
