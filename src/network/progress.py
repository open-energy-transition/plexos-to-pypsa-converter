...
class ProgressTracker:
    ...
    def __init__(
        self, total_items: int, description: str, config: ProgressConfig | None = None
    ):
    ...
    def update(self, current: int, item_name: str | None = None) -> None:
        ...
        if self.config.show_progress and (
            current - self.last_progress_update >= self.config.update_interval
            or current >= self.items_to_process
        ):
            self._display_progress(current, item_name)
            self.last_progress_update = current

    def _display_progress(self, current: int, item_name: str | None = None) -> None:
        ...
        progress_line = (
            f"\r  {self.description}: {current}/{self.items_to_process} "
    ...
    def finish(self, items_added: int | None = None) -> None:
...
def create_progress_tracker(
    ...
    """Create a ProgressTracker with convenience parameters.
    ...
    config = ProgressConfig(
        testing_mode=testing_mode, test_limit=test_limit, show_progress=show_progress
    )
