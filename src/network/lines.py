...
def add_lines_from_plexos(network: Network, db: PlexosDB, default_s_nom: float = 1000.0) -> int:
    """
    ...
    try:
        ...
        for line_name in line_objects:
            ...
            memberships = db.get_memberships_system(line_name, object_class=ClassEnum.Line)
            ...
            if not (node_from and node_to and 
                   node_from in network.buses.index and node_to in network.buses.index):
                logger.debug(f"Skipping line {line_name}: missing nodes {node_from}, {node_to}")
                continue
            ...
            if line_name not in network.lines.index:
                ...
                lines_added += 1
                logger.debug(f"Added line {line_name}: {node_from}-{node_to}, "
                           f"s_nom={s_nom:.1f}MW, r={r:.4f}, x={x:.4f}")
            else:
                logger.warning(f"Line {line_name} already exists, skipping")
                
    except Exception as e:
        logger.error(f"Error adding lines from PLEXOS: {e}")
        raise
...
def _calculate_line_impedance(line_name: str, node_from: str, node_to: str) -> tuple[float, float]:
    """
...
def _is_international_line(node_from: str, node_to: str) -> bool:
    ...
    country_codes = ['AT', 'BE', 'BG', 'CH', 'CZ', 'DE', 'DK', 'ES', 'FI', 
                     'FR', 'GB', 'GR', 'HR', 'HU', 'IE', 'IT', 'LU', 'NL', 
                     'NO', 'PL', 'PT', 'RO', 'SE', 'SI', 'SK']
    
    return (from_country in country_codes and to_country in country_codes and 
...
def set_line_flow_limits(network: Network, db: PlexosDB, 
                        timeslice_csv: Optional[str] = None) -> None:
    """
    ...
    try:
        # Import and use existing flow parsing logic
        ...
        class MockNetwork:
            def __init__(self, real_network):
                ...
                mock_links_data = []
                for line_name in real_network.lines.index:
                    mock_links_data.append({'name': line_name})
        ...
        logger.error(f"Error setting line flow limits: {e}")


def port_lines(network: Network, db: PlexosDB, 
               default_s_nom: float = 1000.0,
               timeslice_csv: Optional[str] = None) -> Dict[str, Any]:
    """
