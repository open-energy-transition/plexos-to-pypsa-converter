...
PLEXOS_DEFAULT_SCHEMA = fpath = (
    files("plexosdb").joinpath("schema.sql").read_text(encoding="utf-8-sig")
)
...
class PlexosDB:
    ...
    def __init__(
        ...
        **kwargs : dict
        ...
        """
        sqlite_kwargs = {
            key: value for key, value in kwargs.items() if key in SQLITE_BACKEND_KWARGS
        }
    ...
    def _get_plexos_version(self) -> tuple[int, ...] | None:
        ...
        try:
            result = self._db.fetchone(
                "SELECT value FROM t_config WHERE element = 'Version'"
            )
    ...
    @classmethod
    def from_xml(
        cls, xml_path: str | Path, schema: str | None = None, **kwargs
    ) -> "PlexosDB":
        ...
        **kwargs : dict
        ...
        xml_tags = set(
            [e.tag for e in xml_handler.root]
        )  # Extract set of valid tags from xml
        for tag in xml_tags:
            ...
            for columns, records in column_groups.items():
                ...
                if not insert_result:
                    logger.warning(
                        f"No rows inserted for {tag} with columns {column_names}"
                    )
    ...
    def add_attribute(
        ...
        *,
        ...
        query = f"INSERT INTO {Schema.AttributeData.name}(object_id, attribute_id, value) VALUES({placeholders})"
    ...
    def add_category(self, class_enum: ClassEnum, /, name: str) -> int:
        ...
        if self.check_category_exists(class_enum, name):
            logger.debug(
                "Category `{}` for `{}` already exist. Returning id instead.",
                name,
                class_enum,
            )
        ...
        rank = (
            self.get_category_max_id(class_enum) or 1
        )  # Default to max rank of 1 if no category exists
    ...
    def add_membership(
        ...
        collection_id = self.get_collection_id(
            collection_enum, parent_class_enum, child_class_enum
        )
    ...
    def add_object(
        ...
        *,
        ...
        _ = self.add_membership(
            ClassEnum.System, class_enum, "System", name, collection_enum
        )
        return object_id

    def add_objects(
        self, *object_names, class_enum: ClassEnum, category: str | None = None
    ) -> None:
    ...
    def add_properties_from_records(
        ...
        *,
        ...
        memberships = self.get_memberships_system(
            component_names, object_class=object_class
        )
        ...
        with self._db.transaction():
            ...
            for property_id in filter_property_ids:
                self._db.execute(
                    "UPDATE t_property set is_dynamic=1 where property_id = ?",
                    (property_id,),
                )
                self._db.execute(
                    "UPDATE t_property set is_enabled=1 where property_id = ?",
                    (property_id,),
                )
        ...
        logger.debug(
            f"Successfully processed {len(records)} property records in batches"
        )
        return
    ...
    def add_property(
        ...
        *,
        ...
        membership_id = self.get_membership_id(
            parent_object_name or "System", object_name, collection_enum
        )
    ...
    def check_category_exists(self, class_enum: ClassEnum, name: str) -> bool:
        ...
        """
        query = (
            f"SELECT 1 FROM {Schema.Categories.name} WHERE name = ? AND class_id = ?"
        )
        class_id = self.get_class_id(class_enum)
    ...
    def check_property_exists(
        ...
        *,
        ...
        if invalid:
            logger.error(
                "Invalid properties {} for collection {}",
                property_names,
                collection_enum,
            )
            return False
    ...
    def copy_object(
        ...
        category_id = self.query(
            "SELECT category_id from t_object WHERE object_id = ?", (object_id,)
        )
        category = self.query(
            "SELECT name from t_category WHERE category_id = ?", (category_id[0][0],)
        )
        new_object_id = self.add_object(
            object_class, new_object_name, category=category[0][0]
        )
        ...
        if not membership_mapping:
            ...
            system_membership_id = self.get_object_memberships(
                ...
                class_enum=object_class,
                include_system_membership=True,
            )[0]["membership_id"]
    ...
    def copy_object_memberships(
        self, object_class, original_name: str, new_name: str
    ) -> dict[int, int]:
        """Copy all existing memberships of old object to the new object."""
        membership_mapping: dict[int, int] = {}
        all_memberships = self.get_object_memberships(
            original_name, class_enum=object_class
        )
        for membership in all_memberships:
            ...
            if child_name == original_name:
                ...
                try:
                    new_id = self.add_membership(
                        parent_class, child_class, parent_name, new_name, collection
                    )
                ...
                try:
                    new_id = self.add_membership(
                        parent_class, child_class, new_name, child_name, collection
                    )
                    membership_mapping[old_id] = new_id
                except Exception as e:
    ...
    def _copy_object_properties(self, membership_mapping: dict[int, int]):
        ...
        with self._db.transaction():
            ...
            self._db.execute(
                "CREATE TEMPORARY TABLE temp_mapping (old_id INTEGER, new_id INTEGER)"
            )

            for old_id, new_id in membership_mapping.items():
                self._db.execute(
                    "INSERT INTO temp_mapping VALUES (?, ?)", (old_id, new_id)
                )
            ...
            self._db.execute(
                "CREATE TEMPORARY TABLE temp_data_mapping (old_id INTEGER, new_id INTEGER)"
            )
    ...
    def delete_object(
        self, object_name: str, /, *, class_enum: ClassEnum, cascade: bool = True
    ) -> None:
    ...
    def get_collection_id(
        ...
        """
        result = self._db.fetchone(
            query, (collection, parent_class_enum, child_class_enum)
        )
        assert result
        return result[0]
    ...
    def get_memberships_system(
        ...
        *object_names: Iterable[str] | str,
        ...
        -------
        list[dict]
            A list of dictionaries representing memberships of the object to the system,
            including both IDs and names for parent_class, parent_object, collection,
            child_class, and child_object.
        ...
        names = normalize_names(*object_names)
        object_ids = tuple(
            self.get_object_id(object_class, name=name, category=category)
            for name in names
        )
        query_string = """
            SELECT
    ...
    def get_object_properties(  # noqa: C901
        ...
        *,
        ...
        if not self.has_properties(
            class_enum, name, collection_enum=collection_enum, category=category
        ):
    ...
    def import_from_csv(
        self, source_path: str | Path, /, *, tables: list[str] | None = None
    ) -> None:
    ...
    def list_objects_by_class(
        self, class_enum: ClassEnum, /, *, category: str | None = None
    ) -> list[dict]:
    ...
    def query(
        self, query_string: str, params: tuple[Any, ...] | dict[str, Any] | None = None
    ) -> list:
    ...
    def search_membership_id(self, membership_id):
        ...
        """
        results = self.query(query, (membership_id,))
        return results
    ...
    def to_csv(
        self, target_path: str | Path, /, *, tables: list[str] | None = None
    ) -> None:
    ...
    def to_xml(self, target_path: str | Path) -> bool:
        ...
        tables = [
            table[0]
            for table in self._db.iter_query(
        ...
        for table_name in tables:
            ...
            if not rows:
                continue
            column_types_tuples = self.query(
                f"SELECT name, type FROM pragma_table_info('{table_name}')"
            )
            column_types: dict[str, str] = {
                key: value for key, value in column_types_tuples
            }
    ...
    def update_category(
        self, category: str, new_name: str, /, *, class_name: ClassEnum
    ) -> None:
