...
def create_model_from_xml(
    ...
    **override_paths
    ...
    if setup_summary.get("format_type") == "iteration":
        print(f"  Iterations processed: {setup_summary.get('iterations_processed', 'N/A')}")
    ...
    if hasattr(network.storage_units_t, 'inflow') and len(network.storage_units_t.inflow.columns) > 0:
...
def create_aemo_model_data_driven(xml_file_path: str, main_directory: Optional[str] = None) -> pypsa.Network:
    """
...
def create_caiso_model_data_driven(xml_file_path: str, main_directory: Optional[str] = None) -> pypsa.Network:
    """
...
def create_sem_model_data_driven(xml_file_path: str, main_directory: Optional[str] = None) -> pypsa.Network:
    """
...
def validate_discovered_paths(discovery_result: Dict, main_directory: str) -> Dict[str, bool]:
    """
...
if __name__ == "__main__":
    ...
    print(f"Network summary: {len(network.buses)} buses, {len(network.generators)} generators")